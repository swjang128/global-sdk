...
public class AccountController {
    ...
    @ApiResponse(responseCode = "200", description = "Success")
    @GetMapping("/{id}")
    public RestApiResponse<AccountGetResponseDto> get(@PathVariable Long id) {
        return accountService.get(id);
    }

    @Operation(summary = "접속 계정 정보 조회 API", description = "계정 로그인여부 확인")
    @ApiResponse(responseCode = "200", description = "Success")
    @GetMapping
    @LogIn
    public RestApiResponse<AccountGetResponseDto> get(Authentication authentication) {
        return accountService.get(authentication.getName());
    }
}
